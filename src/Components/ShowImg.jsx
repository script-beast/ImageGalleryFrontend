...
export default function ShowImg() {
  ...
  async function fthallimg() {
    const resImgs = await fetch(BaseURL + "/show/" + id);
  ...
  async function onDelImg() {
    try {
      ...
      const resImgs = await fetch(BaseURL + "/delete/" + id, requestOptions);
      const dataImgs = await resImgs.json();
          ...
          return (
            <figure key={img._id} className="image-block">
              ...
              <figcaption>
                ...
                >
                  Edit <i className="fas fa-edit mx-2"></i>
                </Link>
                <button
                  type="button"
                  onClick={onDelImg}
                ...
                >
                  Delete <i className="fas fa-trash-alt mx-2"></i>
                </button>
              </figcaption>
            </figure>
          );
